...
function initPlanejamentoDiarioPage() {
    ...
    function renderTasks() {
        ...
        tasksToRender.forEach(task => {
            ...
            window.addMicButtonTo(`#desc-wrapper-${task.id}`, `desc-text-${task.id}`);
            window.addMicButtonTo(`#intent-wrapper-${task.id}`, `intent-text-${task.id}`);
        });
    ...
    window.addMicButtonTo('#wrapper-daily-reflection', 'daily-reflection');
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!process.env.API_KEY) {
        ...
        try {
            ...
            window.showToast("Falha ao inicializar a API do Gemini. Verifique a chave.", "error");
        }
    }
