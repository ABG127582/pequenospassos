...
declare global {
    interface Window {
        ...
        toggleFieldMic: (event: MouseEvent, targetInputId: string, micButtonId: string) => void;
        addMicButtonTo: (wrapperId: string, targetInputId: string, sectionSpecificClass?: string) => void;
        initFisicaPage: () => void;
        initMentalPage: () => void;
...
async function getAISuggestionForInput(prompt: string, targetInput: HTMLInputElement | HTMLTextAreaElement, button: HTMLButtonElement): Promise<void> {
    ...
    try {
        ...
        if (error.toString().includes('403')) {
            window.showToast("Erro de permissão (403). Verifique sua chave de API e as configurações no Google Cloud.", "error");
        } else {
            window.showToast("Ocorreu um erro ao buscar a sugestão. Tente novamente.", "error");
        }
    } finally {
...
async function generateAndDisplayWebResources(button: HTMLElement, loadingEl: HTMLElement, outputEl: HTMLElement, prompt: string) {
    ...
    try {
        ...
        if (error.toString().includes('403')) {
            ...
            window.showToast("Ocorreu um erro ao buscar recursos. Tente novamente.", "error");
        }
...
function initFisicaPage() {
    window.setupListManagement({ sectionKey: 'fisica', storageKey: 'fisicaGoals', listId: 'fisica-metas-list', formId: 'fisica-metas-form', textInputId: 'fisica-meta-input', itemType: 'goal' });
...
function initPlanejamentoDiarioPage() {
    ...
    function renderTasks() {
        ...
        tasksToRender.forEach(task => {
            ...
            taskItem.innerHTML = `
                <div class="task-main-info">
                    ...
                    <div class="task-intention-wrapper input-mic-wrapper planner-mic-wrapper">
                         <textarea class="task-intention" rows="1" placeholder="Intenção (o porquê)...">${task.intention}</textarea>
                         <button type="button" class="ai-suggestion-btn small-button" title="Sugerir Intenção com IA"><i class="fas fa-wand-magic-sparkles"></i><i class="fas fa-spinner fa-spin"></i></button>
                    </div>
            ...
            const descriptionTextarea = taskItem.querySelector('.task-description') as HTMLTextAreaElement;
            const intentionTextarea = taskItem.querySelector('.task-intention') as HTMLTextAreaElement;
            ...
            const suggestBtn = taskItem.querySelector('.ai-suggestion-btn') as HTMLButtonElement;
            ...
            [descriptionTextarea, intentionTextarea].forEach(textarea => {
                textarea?.addEventListener('input', () => { autogrowTextarea(textarea); if(textarea) saveTaskField(task.id, textarea.classList.contains('task-description') ? 'description' : 'intention', textarea.value); });
            ...
            if (suggestBtn && intentionTextarea) {
                 suggestBtn.addEventListener('click', () => {
                    ...
                    window.getAISuggestionForInput(prompt, intentionTextarea, suggestBtn);
                });
            }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!process.env.API_KEY) {
        ...
        warning.innerHTML = '<strong>Atenção:</strong> A chave da API do Gemini não foi encontrada. As funcionalidades de IA estão desativadas. <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer">Obtenha uma chave aqui.</a>';
        ...
        try {
            ...
            const warning = document.createElement('div');
            warning.className = 'api-key-warning';
            warning.innerHTML = '<strong>Erro:</strong> Falha ao inicializar a API do Gemini. Verifique a chave e as configurações.';
            document.body.prepend(warning);
        }
    }
    ...
    document.body.addEventListener('click', async (e: MouseEvent) => {
        ...
        if (backButton) {
            ...
            await window.showPage(targetPage);
    ...
    if (window.updateRainSoundButtonPosition) window.updateRainSoundButtonPosition();
    window.addEventListener('resize', window.updateRainSoundButtonPosition);
